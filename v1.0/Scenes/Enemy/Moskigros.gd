...
func _ready():
	...
	# Changer l'image du Sprite en fonction de is_red
	var autoloader = get_node("/root/Autoloader")
	if autoloader.is_red:
		sprite.texture = load("res://Scenes/Enemy/Moskigros_violet.png")
	else:
		sprite.texture = load("res://Scenes/Enemy/Moskigros.png")

	# Récupérer la valeur d'échelle depuis Autoloader
	var scale_factor = autoloader.moskigros_scale_percent
...
func calculate_movement():
	...
	movement_speed = rand_range(Autoloader.moskigros_speed_min, Autoloader.moskigros_speed_max)
...
func _on_Area2D_body_entered(body):
	...
	if body.is_in_group("ball"):
		...
		var autoloader = get_node("/root/Autoloader")
		autoloader.update_score(autoloader.moskigros_pts_value)
		queue_free()
